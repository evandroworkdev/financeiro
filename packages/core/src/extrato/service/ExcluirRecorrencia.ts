...
export default class ExcluirRecorrencia implements CasoDeUso<string, void> {
  ...
  async executar(recorrenciaId: string, usuario: Usuario): Promise<Resultado<void>> {
    ...
    const existeParcelaConsolidada = gerarExtratosAlterados.instancia.some((e) => e === null);
    if (existeParcelaConsolidada) {
      return Resultado.falha("EXISTE_PARCELA_CONSOLIDADA");
    }
  ...
  private extratosAlterados(extrato: Extrato, recorrencia: Recorrencia): Resultado<Extrato | null> {
    ...
    extrato.removerTransacao(parcela.id);

    return Resultado.ok(extrato);
  }
}
